...
import static frc.robot.generated.TunerConstants.fieldLength;
import static frc.robot.generated.TunerConstants.fieldWidth;
...
public class PhotonRunnable implements Runnable {
      ...
      for (int i = 0; i < signaledHandles.length; i++) {
        ...
        photonResults.forEach(photonResult -> {
          if (photonResult.hasTargets() && (photonResult.targets.size() > 1
              || (photonResult.targets.get(0).getPoseAmbiguity() < Vision.apriltagAmbiguityThreshold))) {
            ...
            photonPoseEstimator.update(photonResult).ifPresent(estimatedRobotPose -> {
              ...
              if (estimatedPose.getX() > 0.0 && estimatedPose.getX() <= fieldLength.in(Meters)
                  && estimatedPose.getY() > 0.0 && estimatedPose.getY() <= fieldWidth.in(Meters)) {
  ...
  private static Matrix<N3, N1> getEstimationStdDevs(
    ...
    if (numTags > 1) {
      estStdDevs = Vision.kMultiTagStdDevs;
    }

    // Increase std devs based on (average) distance
    if (numTags == 1 && avgDist > Vision.singleTagDistanceThreshold.in(Meters)) {
